...
# se commitea. Mismo script que sistema-diseno-almacenes, adaptado.
set -euo pipefail

APP_NAME="sistema_red_distribucion"
...
cp "$SCRIPT_DIR/sistema-red-distribucion.desktop" "$APPDIR/$APP_NAME.desktop"
...
ARCH=x86_64 "$APPIMAGETOOL" --appimage-extract-and-run "$APPDIR" \
  "$OUT_DIR/RedDeDistribucion-x86_64.AppImage"

echo "==> Listo: $OUT_DIR/RedDeDistribucion-x86_64.AppImage"
