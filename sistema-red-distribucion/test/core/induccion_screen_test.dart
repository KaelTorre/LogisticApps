...
import 'package:sistema_red_distribucion/core/tour/induccion_screen.dart';
...
void main() {
  ...
  testWidgets('empieza en el paso 1 y "Siguiente" avanza sin romperse', (tester) async {
    ...
    expect(find.text('Bienvenido al Sistema de Red de Distribución'), findsOneWidget);
        ...
        home: Builder(
          builder: (context) => Scaffold(
            body: Center(
              child: FilledButton(
                onPressed: () =>
                    Navigator.of(context).push(MaterialPageRoute(builder: (_) => InduccionScreen(tour: tour))),
  ...
  testWidgets('recorre todos los pasos hasta el final sin excepciones, y "Terminar" marca vista', (tester) async {
    final tour = await tourVacio();
    await tester.pumpWidget(MaterialApp(home: InduccionScreen(tour: tour)));
    await tester.pumpAndSettle();

    // Bound generoso: evita un bucle infinito por error de programación en
    // vez de fallar con una pista clara si algún día se agregan pasos.
    for (var i = 0; i < 40; i++) {
