#!/usr/bin/env bash
# Empaquetado: arma un .AppImage portable de un solo archivo a partir de
# `flutter build linux --release`. Todo el output intermedio (AppDir, el
# propio appimagetool descargado) vive bajo build/appimage/, que ya está
# cubierto por el /build/ del .gitignore raíz -- nada de esto se
# commitea. Mismo script que sistema-diseno-almacenes/sistema-red-
# distribucion, adaptado.
set -euo pipefail

APP_NAME="sistema_control_logistico"
...
(cd "$PROJECT_ROOT" && flutter build linux --release --no-tree-shake-icons)
...
cp "$SCRIPT_DIR/sistema-control-logistico.desktop" "$APPDIR/$APP_NAME.desktop"
...
ARCH=x86_64 "$APPIMAGETOOL" --appimage-extract-and-run "$APPDIR" \
  "$OUT_DIR/ControlLogistico-x86_64.AppImage"

echo "==> Listo: $OUT_DIR/ControlLogistico-x86_64.AppImage"
